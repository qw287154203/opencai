...
class InterfaceController extends Controller{
	private $cpType = ['lhc','ssc','pk10','syxw','klc','k3','pcdd','qx','dpc'];
	...
	public function current(Request $request,$type,$id){
		$this->istype($type,$id);
		$currentData = \DB::table($id)->orderBy('id','desc')->limit('20')->get();
		if($currentData !== null){
			echo json_encode($this->dodata($currentData));
		}else{
			echo 'null';
		}
	}
	public function history(Request $request,$type,$id,$time){
		$this->istype($type,$id);
		$preg = '/\d{4}-\d{2}-\d{2}/';
		if(!preg_match($preg,$time)){
			die('null');
		}
		$data = \DB::table($id)->whereDate('openTime',$time)->orderBy('id','desc')->get();
		if($data !== null){
			foreach($data as $v){
	...
	public function trend(Request $request,$type,$id,$number){
		$this->istype($type,$id);
		$number = $number > 100 ? '100' : $number;
		$data = \DB::table($id)->orderBy('id','desc')->limit($number)->get();
		if($data !== null){
			foreach($data as $v){
				$count = intval((count($v)-4)/2);
				$num = array_splice($v,0,$count+4);
	...
	private function istype($type,$id){
		if((array_search($type,$this->cpType) === false) or (array_search($id,$this->cpId) === false)){
			die('null');
		}
	}
	private function dodata($data){
		...
		$lottery['tips'] = $lottery['tips'] ?? 'null';
		return $lottery;
	}
}
 ?>
