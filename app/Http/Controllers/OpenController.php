...
class OpenController extends Controller{
	...
	public function index(Request $request,$id){
		$this->istype($id);
		$currentData = $this->dodata(\DB::table($id)->orderBy('id','desc')->limit('20')->get()->toarray(),$id);
		// echo '<pre>';
		// print_r($currentData);die;
		return view($this->cpClass[$id],['data'=>$currentData]);
	}
	public function mindex(Request $request,$id){
		$this->istype($id);
		$currentData = $this->dodata(\DB::table($id)->orderBy('id','desc')->limit('20')->get()->toarray(),$id);
		// echo '<pre>';
		// print_r($currentData);die;
		return view('/m\/'.$this->cpClass[$id],['data'=>$currentData]);
	}
	public function data(Request $request,$id){
		$this->istype($id);
		$currentData = \DB::table($id)->orderBy('id','desc')->limit('20')->get();
		if($currentData !== null){
			// echo'<pre>'; print_r($this->dodata($currentData,$id));die;
			echo json_encode($this->dodata($currentData,$id));
		}else{
			echo 'null';
		}
	}
	...
	private function dodata($data,$id){
		...
		$lottery['cpinfo']['cpname'] = $this->cpName[$id];
		$lottery['cpinfo']['cpid'] = $id;
		return $lottery;
	}
}
 ?>
